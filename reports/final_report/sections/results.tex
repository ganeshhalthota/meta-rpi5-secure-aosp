...
\subsection{Boot Stage Markers and Overhead Analysis}

Diagnostic markers (\texttt{RPi5-DIAG}) were introduced into both the kernel and AOSP \texttt{init} to measure the performance overhead of Android Verified Boot. The kernel diagnostic markers were visible in the serial log, confirming each boot-layer transition:
...
Table~\ref{tab:boot_times} compares the boot progression of a standard non-secure image against a securely signed image. An initial non-secure build was captured using the verbose \texttt{initcall\_debug} profile to demonstrate the UART bottleneck.

\begin{table}[htpb]
\centering
\caption{Boot Time \& Stage Delta Comparison}
\label{tab:boot_times}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llrr}
\toprule
\textbf{Boot Stage} & \textbf{Non-Secure (Verbose)} & \textbf{Non-Secure (Standard)} & \textbf{Secure Boot*} \\
\midrule
\textbf{Layer 1} (\texttt{start\_kernel}) & \texttt{0.000 s} & \texttt{0.000 s} & \texttt{0.000 s} \\
\textbf{Layer 2} (\texttt{console\_init}) & \texttt{0.000 s} (+0 ms) & \texttt{0.000 s} (+0 ms) & \texttt{0.000 s} (+0 ms) \\
\textbf{Layer 3} (\texttt{do\_initcalls}) & \texttt{0.009 s} (+9 ms) & \texttt{0.008 s} (+8 ms) & \texttt{0.009 s} (+9 ms) \\
\textbf{Layer 4} (\texttt{kernel\_init}) & \texttt{22.675 s} (+22.666 s) & \texttt{1.119 s} (+1.111 s) & \texttt{1.115 s} (+1.106 s) \\
\textbf{AOSP Stage 2} (\texttt{SetupSelinux}) & \texttt{23.072 s} (+397 ms) & \texttt{1.348 s} (+229 ms) & \texttt{1.609 s} (+494 ms) \\
\textbf{AOSP Stage 4} (\texttt{Boot Complete}) & \texttt{67.677 s} (+44.605 s) & \texttt{43.527 s} (+42.179 s) & \textbf{DNF} (Crash Loop) \\
\bottomrule
\multicolumn{4}{l}{\footnotesize *Note: The secure log represents a boot attempt that fails to start the UI framework.}
\end{tabular}%
}
\end{table}
...
\textbf{Key Findings:}
\begin{itemize}
    \item \textbf{UART Logging Bottleneck (Layer 3 $\rightarrow$ 4):} The verbose kernel driver probes delayed the handoff to AOSP by over 21~seconds. Disabling the debug profile normalises the driver probe phase to $\sim$1.11~seconds for both builds.
    \item \textbf{AVB Verification Overhead (Layer 4 $\rightarrow$ AOSP Stage 2):} This phase involves Android's \texttt{FirstStageMain} mounting and verifying the early partitions. The clean non-secure build mounts them in 229~ms, while the secure build takes 494~ms. The 265~ms difference reflects the exact computational cost of validating \texttt{vbmeta} signatures and initializing \texttt{dm-verity} hashtrees on the RPi5.
    \item \textbf{Framework Boot (AOSP Stage 2 $\rightarrow$ 4):} The non-secure framework successfully boots in 42.1~seconds. The secure build fails to complete this phase due to a crash loop (investigated further in Section~\ref{sec:impl_fbe_selinux}).
\end{itemize}
